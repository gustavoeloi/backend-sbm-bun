...
export const getDayOrdersAmount = new Elysia().use(auth).get('/metrics/day-orders-amount', async ({ cookie: { authCookie }, jwt, }) => {
  ...
  const orderPerDay = await db.select({
    dayWithMonthAndYear: sql<string>`TO_CHAR(${orders.createdAt}, 'YYYY-MM-DD')`,
    amount: count()
  }).from(orders).where(and(eq(orders.establishmentId, payload.establishmentsId), gte(orders.createdAt, startOfYesterday.toDate()))).groupBy(sql`TO_CHAR(${orders.createdAt}, 'YYYY-MM-DD')`)
  ...
  return {
    ...
    diffFromLastDay: diffFromYesterday ? Number((diffFromYesterday - 100).toFixed(2)) : 0
  }
