...
export const getMonthOrdersAmount = new Elysia().use(auth).get('/metrics/month-orders-amount', async ({ cookie: { authCookie }, jwt, }) => {
  ...
  const orderPerMonths = await db.select({
    monthWithYear: sql<string>`TO_CHAR(${orders.createdAt}, 'YYYY-MM')`,
    amount: count()
  }).from(orders).where(and(eq(orders.establishmentId, payload.establishmentsId), gte(orders.createdAt, startOfLastMonth.toDate()))).groupBy(sql`TO_CHAR(${orders.createdAt}, 'YYYY-MM')`)
  ...
  const diffFromLastMonth = currentMonthOrdersAmount && lastMonthOrdersAmount ? (currentMonthOrdersAmount.amount * 100) / lastMonthOrdersAmount.amount : null
